...
```shell
wget --no-check-certificate -O install.sh https://raw.githubusercontent.com/leader-gzcn/IBMYes/master/install.sh && chmod +x install.sh  && ./install.sh
```

![image-20200615210944753](img/README/image-20200615210944753.png)

在配置的时候需要输入应用名称（这里就是我创建应用的时候输入应用名称我输入的是ibmyes，你需要改成你自己的名称）、应用内存大小（我们刚刚选择的是256）、v2ray的UUID（默认的会随机产生）、v2ray的PATH（默认的会随机产生）。
...
打开项目下的`better-cloudflare-ip-win32`目录运行`cf.bat`，按照提示输入期望的带宽大小，程序会一直测试直到找到满足要求的IP，建议从30起步。
