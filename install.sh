...
create_mainfest_file(){
	...
	read -p "请输入v2ray的UUID(默认随机产生)：" UUID
	if [ -z "${IBM_MEM_SIZE}" ];then
    UUID=$(cat /proc/sys/kernel/random/uuid)
    fi
    echo "v2ray的UUID：${UUID}"
	read -p "请输入v2ray的PATH(默认随机产生)：" WSPATH
    if [ -z "${WSPATH}" ];then
    WSPATH=$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 6)
    fi
    echo "v2ray的PATH：${WSPATH}"
...
EOF
    ...
    cat >  ${SH_PATH}/IBMYes/v2ray-cloudfoundry/web/config.json  << EOF
    {
        "inbounds": [
            {
                "port": 8080,
...
clone_repo(){
    echo "进行初始化。。。"
	rm -rf IBMYes
    git clone https://github.com/leader-gzcn/IBMYes
    cd IBMYes
    git submodule update --init --recursive
    cd v2ray-cloudfoundry/web
